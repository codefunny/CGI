...
//        All rights reserved
//
//        filename : api-receiving.cpp
//        description : 收货的CGI入口
//
...
{
    string strCGIName = "/api/receiving";
    ...
    vector<unsigned int> vecOrderIDReceivedList;
    unsigned int iGetinGoodsResult = logic::GetinGoods(oRequest.strUserID, oRequest.strUserKey, tools::CStringTools::Split2Uint(oRequest.strOrderIDList, "#"), vecOrderIDReceivedList);
    if (error_num::SUCCEED == iGetinGoodsResult)
    {
        std::string strData;
        size_t maxSize = vecOrderIDReceivedList.size();
        for (size_t index = 0; index < maxSize; index++)
        {
            if (index == maxSize - 1)
            {
                strData = strData + tools::CStringTools::Int2String(vecOrderIDReceivedList[index]);
            }
            else
            {
                strData = strData + tools::CStringTools::Int2String(vecOrderIDReceivedList[index]) + "#";
    ...
    {
        jsonOutput["ret"] = "1";
        jsonOutput["errcode"] = tools::CStringTools::Int2String(iGetinGoodsResult);
        jsonOutput["errstr"] = "收货失败";
    }
