...
//        All rights reserved
//
//        filename : api-modify_pwd.cpp
//        description : 修改密码的CGI入口
//
...
static bool ParseCgi(CRequest& oRequest)
{
    ...
    if (!oRequest.strPostData.empty())
    {
        ...
        if (mapRequest.end() == mapRequest.find("oldpwd"))
        {
            ERROR("invalid request");
            return false;
        }
        if (mapRequest.end() == mapRequest.find("newpwd"))
        {
            ERROR("invalid request");
            return false;
        }
        ...
        oRequest.strOldpwd = mapRequest["oldpwd"];
        oRequest.strNewpwd = mapRequest["newpwd"];
    }
...
{
    string strCGIName = "/api/modify_pwd";
    ...
    string strUserID;
    string strUserKey;
    if (user::ModifyPassword(iChannelIDAfterConvert, oRequest.strName, oRequest.strOldpwd, oRequest.strNewpwd, strUserID, strUserKey))
    {
        DEBUG("modify successfully");
    ...
    else
    {
        ERROR("modify failed");
        jsonOutput["ret"] = "1";
        jsonOutput["errcode"] = tools::CStringTools::Int2String(error_num::USER_MODIFY_FAILED);
        jsonOutput["errstr"] = "修改密码失败";
        return;
    }
...
int main()
{
    while(FCGI_Accept() >= 0)
    {
        ...
        ssRequest << "==========CGI START==========\n"
                  ...
                  << "oldpwd            is : " << oRequest.strOldpwd << "\n"
                  << "newpwd            is : " << oRequest.strNewpwd << "";
        DEBUG(ssRequest.str());
